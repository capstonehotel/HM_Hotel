...
if (isset($_POST['otp'])) {
    ...
    if (isset($_SESSION['otp_email']) && $_SESSION['otp_email'] == $email) {
        ...
        if (preg_match('/^[0-9]{6}$/', $userOtp)) {
            ...
            if (isset($_SESSION['otp']) && $_SESSION['otp'] !== '' && $_SESSION['otp'] === $userOTP) {
                ...
                echo $userOTP.'OTP not found in session or OTP is empty or OTP is invalid'.$_SESSION['otp']; // Handle the case where the OTP is not found in the session or OTP is empty or OTP is invalid
            }
        } else {
